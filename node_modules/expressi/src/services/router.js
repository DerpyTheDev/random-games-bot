...
const parseRoutes = ({ location, files }) => {
  return files.map(file => {
    ...
    const hasCustomMethod = route.indexOf('--') > -1

    if (hasCustomMethod) {
      const splittedRoute = route.split('--')
      route = splittedRoute[0]
      method = splittedRoute[1]
    }
    ...
    console.log(`${emoji.get('white_check_mark')}   ${method.toUpperCase()} ${chalk.blue(route)}`)
...
const attachRouteToExpress = (app, routes) => {
  routes.forEach(({ route, file, method }) => {
    app[method](route, require(file).default)
  })
}
